...
| 1 Program, Today, workout mode, backup | Built and deployed 2026-09-02 (build ebfd42b), awaiting your gym check | Open Summit from the home screen on a lifting day, tap Start workout, log every set with the Complete button, let the rest timer run once, tap Finish, pick a status, tap Save workout. Then tell me what was wrong, slow, or missing |
...
- Live address: build stamp ebfd42b served; index, service worker precache, and asset agree (each answers 200). The service worker now registers at startup so a new build reloads onto the phone from any screen; before this fix it only registered on More.
